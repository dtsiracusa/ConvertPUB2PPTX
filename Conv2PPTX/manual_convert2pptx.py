...
class ConversionLogger:
    ...
    def __init__(self, log_directory=None):
        ...
        if log_directory is None:
            # REPLACE THIS PATH WITH YOUR DESIRED LOG DIRECTORY (Otherwise this will store logs in a folder next to the script)
            log_directory = BASE_DIR / "ConversionLogs"
...
class PublisherToPowerPointConverter:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        # Release COM references explicitly before CoUninitialize.
        # The processes are already force-killed above, so Release()
        # calls will fail fast (RPC server dead) instead of blocking.
        self.publisher = None
        self.powerpoint = None
    ...
    def convert_batch(self, input_paths, output_dir=None, fallback_to_image=True, 
        ...
        for idx, pub_file in enumerate(pub_files, 1):
            ...
            if idx % checkpoint_interval == 0 and idx < total_files:
                self._checkpoint_prompt(idx, total_files, converted_count, output_dir)
    ...
    def _checkpoint_prompt(self, processed, total, converted, output_dir):
        """Prompt user at checkpoint to verify conversions."""
        print("\n" + "="*80)
        print("BATCH VERIFICATION CHECKPOINT")
        print("="*80)
        print(f"Processed: {processed}/{total} files")
        print(f"Successful: {converted}")
        print(f"Failed: {processed - converted}")
        if output_dir:
            print(f"Output directory: {output_dir}")
        print("\nPlease check the converted files before continuing.")
        print("\nOptions:")
        print("  1. Continue with next batch")
        print("  2. Stop conversion (clean exit)")
        print("  3. Continue All (disable further checkpoints)")
        print("="*80)
        ...
        while True:
            try:
                ...
                if choice == '1':
                    ...
                    print("\nStopping conversion. Files processed so far have been saved.")
                    sys.exit(0)
                elif choice == '3':
                    print("\nDisabling checkpoints and continuing...")
                    # Set checkpoint interval to infinity
                    self.convert_batch.__defaults__ = (True, float('inf'))
                    return True
                else:
...
def prompt_skip_or_abort(file_path, error_message):
    """
    Prompt user to skip file or abort entire batch.
    ...
    print("\n" + "="*80)
    print("Options:")
    print("  1. Skip this file (continue with remaining files)")
    print("  2. Abort entire batch (stop all conversions)")
    print("="*80)
...
def prompt_selection_mode():
    """Prompt user to select files or folder."""
    while True:
        print("\nSelect conversion mode:")
        print("  1. Select individual file(s)")
        print("  2. Select entire folder")
...
def find_pub_files_recursive(folder_path):
    """Recursively find all .pub files in a folder."""
    folder = Path(folder_path)
    pub_files = list(folder.rglob("*.pub"))
    
    print(f"\nFound {len(pub_files)} Publisher file(s) in folder:")
    for f in pub_files:
        print(f"  - {f.name}")
    
    return [str(f) for f in pub_files]
...
def select_folder_dialog():
    ...
    folder_path = filedialog.askdirectory(
        title="Select Folder Containing Publisher Files"
    )
...
def select_output_folder():
    ...
    print("\nOpening output folder selection dialog...")
    folder_path = filedialog.askdirectory(
...
def select_files_dialog():
    """Open file selection dialog for Publisher files."""
    root = Tk()
    root.withdraw()
    root.attributes('-topmost', True)
    ...
    file_paths = filedialog.askopenfilenames(
        title="Select Publisher File(s) to Convert",
        filetypes=[
            ("Publisher Files", "*.pub"),
            ("All Files", "*.*")
        ],
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Interactive mode (file dialog + dry-run prompt)
  python pub_to_ppt.py
  
  # Convert specific files (command line)
  python pub_to_ppt.py document.pub
  
  # Convert all .pub files in a directory
  python pub_to_ppt.py C:/MyDocuments/
  
  # Convert with custom output directory
  python pub_to_ppt.py file1.pub file2.pub -o C:/Output/
  
  # Convert using a custom template
  python pub_to_ppt.py document.pub --template "path/to/template.potx"
  
  # Dry run mode (command line)
  python pub_to_ppt.py document.pub --dry-run
  
  # Disable specific content types
  python pub_to_ppt.py document.pub --no-images --no-shapes
  
  # Specify log directory
  python pub_to_ppt.py document.pub --log-dir "C:/MyLogs"
        """
    )
    ...
    parser.add_argument(
        'input',
        nargs='*',
        help='Publisher file(s) or directory containing .pub files (optional - will prompt if not provided)'
    )
    
    parser.add_argument(
        '-o', '--output',
        help='Output directory or filename for converted file(s)'
    )
    # Template path for PowerPoint Template
    parser.add_argument(
        '-t', '--template',
        ### CHANGE ME AS NEEDED ("BASE_DIR" is the folder that the script lives in | "Wieland SOPs Template.potx" is the file name + extension of the template file;
        #  therefore the template should live in the same folder as the script)
        default=BASE_DIR / "Wieland SOPs Template.potx",
        ## Below is a full file path example for reference. If desiring to use this style,
        # default=r"C:\Your Folder Path\Where the Template File Lives\Wieland SOPs Template.potx",
        help='PowerPoint template file (.potx) to use (default: Template for PowerPoint SOPs.potx)'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help='Pause after this many files for verification (default: 5)'
    )
    ...
    parser.add_argument(
        '--dry-run',
        action='store_true',
        help='Dry run mode - show what would be converted without actually converting'
    )
    ...
    logger.log_message("Publisher to PowerPoint Converter Started")
    
    # Interactive mode if no input files provided
    if not args.input:
        print("="*60)
        print("Publisher to PowerPoint Converter")
        print("="*60)
        
        
        # Prompt for selection mode (files or folder)
        ...
        if selection_mode == 'files':
            ...
            if not input_files:
                ...
                logger.log_message("No files selected. Session ended.")
                logger.finalize()
                return
        else:  # folder mode
            ...
            if not folder_path:
                ...
                logger.log_message("No folder selected. Session ended.")
                logger.finalize()
                return
            
            input_files = find_pub_files_recursive(folder_path)
            if not input_files:
                print("\nNo Publisher files found. Exiting.")
                logger.log_message("No Publisher files found. Session ended.")
                logger.finalize()
                return
        
        # Select output folder via dialog
        output_folder = select_output_folder()
        
        if not output_folder:
            print("\nNo output folder selected. Exiting.")
            logger.log_message("No output folder selected. Session ended.")
            logger.finalize()
            return
    ...
    # Determine if output is a directory or filename
    output_dir = None
    output_file = None
    
    if args.output:
        output_path = Path(args.output)
        if len(args.input) == 1 and Path(args.input[0]).is_file() and output_path.suffix == '.pptx':
    ...
    try:
        with PublisherToPowerPointConverter(
            ...
            if len(args.input) == 1 and Path(args.input[0]).is_file() and output_file:
